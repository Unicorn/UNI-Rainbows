...
export default function TabsLayout() {
  return (
    <Tabs
      ...
      tabBar={props =>
        Platform.OS === 'ios' ? (
          <BlurView style={{ position: 'absolute', bottom: 0, left: 0, right: 0 }} intensity={95}>
            <BottomTabBar {...props} />
          </BlurView>
        ) : (
          <BottomTabBar {...props} />
        ...
        options={{
          ...
          tabBarIcon: ({ color }) => (
            ...
            >
              ...
              <Text style={{ marginTop: 5, fontSize: 10, opacity: 0.5 }}>Account</Text>
            </View>
          ),
        }}
      />
...
function TabBarIcon(props: { name: React.ComponentProps<typeof FontAwesome>['name']; color: string; size?: number }) {
  return <FontAwesome size={props.size || 26} style={{ marginBottom: -3 }} {...props} />
}
